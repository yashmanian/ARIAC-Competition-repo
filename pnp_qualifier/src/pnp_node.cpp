...
void toggleGripper(ros::NodeHandle & node)
{
  // Create a Service client for the correct service, i.e. '/ariac/gripper/control'.
  ros::ServiceClient vac_client = node.serviceClient<osrf_gear::VacuumGripperControl>("/ariac/gripper/control");
  osrf_gear::VacuumGripperControl vac_srv;  // Combination of the "request" and the "response".
  vac_client.call(vac_srv); // Call service
  //ROS_INFO("Vacuum gripper state:" << vac_srv.enable << "Object attached:" << vac_srv.success);
}
...
public:
  ...
  {
    ROS_INFO_STREAM_THROTTLE(1,"Joint States (throttled to 1 Hz):\n" << *joint_state_msg);
  ...
  {
    ROS_INFO_STREAM_THROTTLE(1,"Logical camera: '" << image_msg->models.size() << "' objects.");
