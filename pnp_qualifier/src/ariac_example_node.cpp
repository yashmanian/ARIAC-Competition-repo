...
//    ROS_INFO_STREAM("order msg:\n" << received_orders_[0].kits[0].objects[0].type);
std::vector<osrf_gear::KitObject> order_listss;
ROS_INFO_STREAM("order msg:\n" << received_orders_[0].kits[0].objects[0]);
order_listss.push_back(received_orders_[0].kits[0].objects[0]);
  }
  ...
  {
const osrf_gear::Model logical_object=image_msg->models[1];
geometry_msgs::Pose logical_camera_pose=image_msg->pose;
geometry_msgs::Pose logical_object_pose=logical_object.pose;
    ROS_INFO_STREAM_THROTTLE(10,
      "Logical camera: '" << logical_object_pose.position.x << "' objects.");
  }
